...
document.addEventListener('DOMContentLoaded', () => {
  ...
  fetch('/api/stats')
    .then(res => res.json())
    .then(data => {
      document.getElementById('stat-dsa').innerText = `${data.algorithmsCount}+`;
      document.getElementById('stat-cheats').innerText = `${data.cheatsheetsCount}+`;
    })
  ...
  function renderDsa(items) {
    ...
    if (!items.length) {
      container.innerHTML = '<p style="color: var(--text-secondary)">No algorithms matched your search.</p>';
      return;
    }
  ...
  function renderCheats(categories) {
    ...
    Object.entries(categories).forEach(([category, items]) => {
      items.forEach(item => {
        let codeContent = item.cmd || item.pattern || `HTTP ${item.code}: ${item.name}`;
  ...
  searchInput.addEventListener('input', (e) => {
    ...
    // Filter DSA
    const filteredDsa = dsaData.filter(item =>
  ...
  runBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      document.getElementById('time-quick').innerText = `${res.quickSort.toFixed(2)} ms`;
      document.getElementById('time-merge').innerText = `${res.mergeSort.toFixed(2)} ms`;
      document.getElementById('time-heap').innerText = `${res.heapSort.toFixed(2)} ms`;

      document.getElementById('bar-quick').style.width = `${Math.min(100, Math.max(10, (res.quickSort / maxTime) * 100))}%`;
      document.getElementById('bar-merge').style.width = `${Math.min(100, Math.max(10, (res.mergeSort / maxTime) * 100))}%`;
      document.getElementById('bar-heap').style.width = `${Math.min(100, Math.max(10, (res.heapSort / maxTime) * 100))}%`;
    } catch (err) {
