...
switch (command) {
  case "big-o":
    ...
    console.log(
      "  Algorithm/Structure   | Best Time   | Average     | Worst       | Space",
    );
    console.log(
      "  --------------------------------------------------------------------------",
    );
  ...
  case "cheat":
    if (!subarg || !Cheatsheets[subarg]) {
      console.log(
        "Available cheatsheet categories: \x1b[32mgit, docker, linux, httpStatus, regex\x1b[0m\n",
      );
      console.log("Example: node bin/devatlas.js cheat git\n");
    } else {
      console.log(
        `\x1b[33m--- CHEATSHEET: ${subarg.toUpperCase()} ---\x1b[0m\n`,
      );
      ...
      list.forEach((item, idx) => {
        if (item.cmd) {
          ...
          console.log(
            `  \x1b[35m[HTTP ${item.code}] ${item.name}\x1b[0m: ${item.desc}`,
          );
        } else if (item.pattern) {
          console.log(
            `  \x1b[36mPattern:\x1b[0m /${item.pattern}/\n    ↳ ${item.desc}\n`,
          );
        }
      });
    }
  ...
  case "search":
    if (!subarg) {
      console.log(
        "Please provide a search term. Example: node bin/devatlas.js search sort",
      );
    } else {
      console.log(`\x1b[33mSearching for: "${subarg}"...\x1b[0m\n`);
      const matches = [];
      Object.keys(Sorting).forEach((name) => {
        if (name.toLowerCase().includes(subarg))
          matches.push(`[Sorting] ${name}`);
      });
      Object.keys(Searching).forEach((name) => {
        if (name.toLowerCase().includes(subarg))
          matches.push(`[Searching] ${name}`);
      });
  ...
  default:
    ...
    console.log(
      "  \x1b[32mbig-o\x1b[0m                  View algorithm complexity cheatsheet",
    );
    console.log(
      "  \x1b[32mcheat <category>\x1b[0m       View cheatsheets (git, docker, linux, regex, httpStatus)",
    );
    console.log(
      "  \x1b[32msearch <keyword>\x1b[0m       Search algorithms in the library",
    );
    console.log(
